...
import com.hiking.humusicplayer.music_service.AudioFocusManager;
import com.hiking.humusicplayer.music_service.MediaSessionManager;
import com.hiking.humusicplayer.music_service.PlayControllerImp;

import java.util.List;

/**
 * Created by Administrator on 2018/4/5.<br><br>
 * 仅有从 {@link com.hiking.humusicplayer.IPlayController}.aidl 继承的方法在跨进程调用时有效<br>
...
public class PlayController extends IPlayController.Stub {
    ...
    private final PlayControllerImp manager;
    ...
    public PlayController(Context context) {
        ...
        this.manager = PlayControllerImp.getMediaController(
                context,
                focusManager,
                sessionManager,
    ...
    public synchronized int play(Song which) {
        if (which == null)
            return -1;
        int re = PlayControllerImp.ERROR_UNKNOWN;
...
//        return manager.setPlaySheet(sheetID, current);
    ...
    public synchronized void setPlayMode(int mode) {
        if (mode >= PlayControllerImp.MODE_DEFAULT && mode <= PlayControllerImp.MODE_RANDOM)
    ...
    private class NotifySongChange implements PlayControllerImp.NotifySongChanged {
    ...
    private class NotifyStatusChange implements PlayControllerImp.NotifyStatusChanged {
        ...
        public void notify(Song song, int index, int status) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (listener != null) {
                    try {
                        switch (status) {
                            case PlayControllerImp.STATUS_START:
                                listener.playStart(song, index, status);
                                break;
                            case PlayControllerImp.STATUS_STOP:
    ...
    private class NotifyPlayListChange implements PlayControllerImp.NotifyPlayListChanged {
